...
const searchDoctorsInput = z.object({
    specialty: z
        .string()
        .optional()
        .describe('Medical specialty as the patient said it, e.g. "cardiologist", "cardiology", "dermatologist", "paediatrician". Matched fuzzily against specialty names and aliases.'),
    city: z
        .string()
        .optional()
...
const compareSlotsInput = z.object({
    doctorIds: z
        ...
        .min(1)
        .max(4)
        .describe('Doctor ids to compare side by side, in display order. Use the `doctorId` values returned by search-doctors, e.g. ["doc_pune_card_01", "doc_pune_card_02"].'),
    date: z
        .string()
...
__decorate([
    Tool({
        ...
        description: 'Find doctors in the Arogya provider directory by specialty and city. Returns doctor cards with photo, hospital, qualifications, consultation fee, rating and live slot availability. Use the returned `doctorId` values for compare-slots and book-appointment.',
...
__decorate([
    Tool({
        ...
        description: 'Compare the open consultation slots of two or more doctors on a specific date, side by side. Returns one column per doctor with every available slot (start time, end time, mode, fee) plus earliest/latest availability. Use the returned `slotId` to book.',
